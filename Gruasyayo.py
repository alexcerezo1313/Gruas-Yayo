...
# Definir los rangos permitidos (siempre mayores o iguales al target)
alcance_min = float(target_alcance)
alcance_max = target_alcance * 1.15  # máximo permitido (15% más)
carga_punta_min = float(target_carga_punta)
carga_punta_max = target_carga_punta * 1.25  # máximo permitido (25% más)
...
if use_stock:
    ...
    for grua in gruas_list:
        # Convertir valores a float para evitar problemas de tipo
        try:
            alcance_val = float(grua.get("Pluma Instalada", 0))
            carga_val = float(grua.get("Carga en Punta", 0))
        except:
            continue
        # Filtrar: deben estar entre target y máximo permitido
        ...
        if not use_intermedia:
            ...
            # Con carga intermedia: exigir que estén dentro del rango [target, target*1.05]
            try:
                dist_val = float(grua.get("Distancia Específica", 0))
                carga_int_val = float(grua.get("Carga específica", 0))
            except:
    ...
    # Modo SIN stock: aplicar criterios de error en alcance y carga en punta
    for grua in gruas_list:
        try:
            alcance_val = float(grua.get("Pluma Instalada", 0))
            carga_val = float(grua.get("Carga en Punta", 0))
            ...
            continue
        # Rechazar si el valor es menor que target o mayor que el máximo permitido
        if not (alcance_min <= alcance_val <= alcance_max):
            continue
        if not (carga_punta_min <= carga_val <= carga_punta_max):
            continue

        # Calcular error relativo (se supone que el valor es mayor o igual al target)
        err_alcance = relative_error(alcance_val, target_alcance)
        err_carga = relative_error(carga_val, target_carga_punta)
        # Descartar si alguno supera el 15%
        ...
        # Determinar Tipo para efectos de estilo:
        # Si ambos errores (de alcance y carga en punta) son ≤ 0.05, se considera Match (verde)
        # Si alguno está entre >0.05 y ≤0.15, se considera Casi Match (amarillo)
...
if not resultados:
    ...
    # En modo SIN stock, agregaremos columnas auxiliares para estilo pero luego se ocultan
    cols_aux = []
    if not use_stock:
    ...
    # En modo SIN stock: se colorea de verde si "Tipo" es Match, de amarillo si es Casi Match.
    def color_rows(row):
        if use_stock:
