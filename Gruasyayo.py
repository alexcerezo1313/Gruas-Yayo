...
if not resultados:
    ...
    # Ocultar las columnas auxiliares "Total Error" y "Tipo" mediante CSS (si hide_columns no está disponible)
    # Creamos estilos para ocultar las celdas de las columnas auxiliares.
    hide_styles = []
    for col in cols_aux:
        if col in df.columns:
            col_index = list(df.columns).index(col)
            # Agregamos reglas CSS para encabezado y celdas
            hide_styles.append({
